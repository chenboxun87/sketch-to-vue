...
try:
    from mastergo_get_dsl import get_dsl_from_url
except ImportError:
    import os
...
def analyze_node(node: Dict, depth: int = 0) -> Dict[str, Any]:
    ...
    if width and height:
        summary["size"] = f"{width.get('value', '?')}x{height.get('value', '?')}"
...
def format_tree(analysis: Dict) -> str:
    lines = []
    lines.append(f"DSL Analysis (v{analysis['version']}, {analysis['framework']})")
    lines.append(
        f"Stats: {analysis['stats']['totalNodes']} nodes, "
    ...
    def print_node(node: Dict, prefix: str = "", is_last: bool = True):
        ...
        line = f"{prefix}{connector}[{node.get('type', '?')}] {node.get('name', 'unnamed')}"
        if node.get("size"):
            line += f" ({node['size']})"
        if node.get("text"):
    ...
    if analysis["texts"]:
        ...
        for item in analysis["texts"][:20]:
            text = item["text"][:80] + ("..." if len(item["text"]) > 80 else "")
            lines.append(f'  [{item["id"]}] {item["name"]}: "{text}"')
...
def format_flat(analysis: Dict) -> str:
    ...
    def flatten(node: Dict, path: str = ""):
        ...
        line = f"[{node.get('type', '?')}] {current_path}"
        if node.get("size"):
            line += f" | {node['size']}"
        if node.get("text"):
            line += f' | "{node["text"][:50]}"'
...
def main():
    parser = argparse.ArgumentParser(description="Analyze MasterGo DSL structure")
    ...
    parser.add_argument("--format", "-f", choices=["tree", "json", "flat"], default="tree")
